...
if ~interactive_visualization
    ...
    [C,i]=sort(bounds_predictions.score,'descend');
        bounds_predictions.score=bounds_predictions.score(i);
        bounds_predictions.bounds=bounds_predictions.bounds(:,i);
        bounds_predictions.poselet_id=bounds_predictions.poselet_id(i);
        bounds_predictions.image_id=bounds_predictions.image_id(i);
        bounds_predictions.src_idx=bounds_predictions.src_idx(i);
        bounds_predictions.bounds(bounds_predictions.bounds<0)=1;   
    
        g=size(bounds_predictions.src_idx{1});
        
 for j=1:g(2)   
 i=bounds_predictions.src_idx{1}(j);
 bound=poselet_hits.bounds(:,i);
 x=floor(poselet_hits.bounds(2,i):poselet_hits.bounds(2,i)+poselet_hits.bounds(4,i));
 y=floor(poselet_hits.bounds(1,i):poselet_hits.bounds(1,i)+poselet_hits.bounds(3,i)); 

 x=x((x<height) & (x>0));
 y=y((y<width) & (y>0));

 img_pos=img(x,y,:);
 name=strcat('poses/',num2str(j),'-pos-act.jpg');
